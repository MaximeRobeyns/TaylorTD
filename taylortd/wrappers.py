...
if version.parse(gym.__version__) > version.parse("0.9.4"):
    ...
    from taylortd.video_recorder import VideoRecorder
...
class BoundedActionsEnv(gym.Wrapper):
    ...
    def __init__(self, env, mode="numpy"):
        ...
        self.action_space = gym.spaces.Box(
            low=-1, high=1, shape=self.unwrapped.action_space.shape
        )
    ...
    def step(self, action):
        if self.mode == "numpy":
            ...
            lb, ub = (
                self.unwrapped.action_space.low,
            ...
            lb, ub = torch.tensor(
                self.unwrapped.action_space.low
            ), torch.tensor(self.unwrapped.action_space.high)
...
class IsDoneEnv(gym.Wrapper):
    ...
    def __init__(self, env, mode="numpy"):
        assert mode in ["numpy", "torch"]
        assert hasattr(
            env.unwrapped, "is_done"
        ), f"{env} has no is_done method to compute done caused by termination conditions"
        super().__init__(env)
        self.mode = mode
    ...
    def step(self, action):
        ...
        with torch.no_grad():  # Just to be sure...
            ...
            done_condition = bool(
                self.env.unwrapped.is_done(t_state).item()
            )  # done do to termination conditions
