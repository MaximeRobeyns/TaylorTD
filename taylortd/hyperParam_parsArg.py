...
parser.add_argument(
    ...
    type=str,
    nargs="?",
    default="=bp1-gpu004,bp1-gpu006,bp1-gpu016,bp1-gpu017",
)
parser.add_argument(
    "--cmd", type=str, nargs="*", help="job command --- must be last argument"
)
...
parser.add_argument(
    ...
    type=bool,
    default=False,
    help="Set to True to do hyp-search over state covariance",
)
parser.add_argument(
    "--action_cov_training",
    "-tac",
    type=bool,
    default=False,
    help="Set to True to do hyp-search over action covariance",
)
...
parser.add_argument(
    ...
    type=str,
    default="trial",
    help="This determine the directory folder to save the data -e.g. test/train/etc",
)
...
if args.grad_state:
    ...
    if args.state_cov_training:
        state_cov_range = torch.linspace(
            0.000001, 0.0001, 5
        )  # Change this range to set range for state_cov hyper-param search
    else:
...
if args.update_order > 0:
    ...
    if args.action_cov_training:
        action_cov_range = torch.linspace(
            0.01, 1, 5
        )  # Change this range to set range for action_cov hyper-param search
    else:
...
for s in seeds:
    ...
    for sc in state_cov_range:
        ...
        for ac in action_cov_range:
            ...
            print(
                cmd,
                " with env_name=GYMMB_" + args.env_name,
                "agent_alg=" + args.agent_alg,
                f"td3_action_cov={ac}",
                f"td3_state_cov={sc}",
                f"n_total_steps={args.n_steps}",
                f"seed={s}",
                f"run_type=" + args.run_type,
                f"run_number={counter}",
                f"td3_update_order={args.update_order}",
                f"grad_state={args.grad_state}",
                f"seed={s}",
            )
            counter += 1
