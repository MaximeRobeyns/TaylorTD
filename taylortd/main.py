...
ex = sacred.Experiment(
    save_git_info=False
)  # Needed to add it to run on BP as don't have python git dependecies...
...
def main_config():
    n_total_steps = (
        100000  # total number of steps in real environment (including warm up)
    )
...
def eval_config():
    ...
    n_eval_episodes_per_policy = (
        10  # number of episodes each eval agent is evaluated for each task
    )
...
def model_training_config():
    ...
    model_weight_decay = (
        1e-4  # L2 weight decay on model parameters (good: 1e-5, default: 0)
    )
...
def policy_training_config(env_name):
    ...
    policy_training_freq = (
        1  # interval (in real steps) between subsequent policy trainings
    )
...
def infra_config(
    ...
    if dump_dir == "__default__":
        ...
        dump_dir = os.path.join(
            ...
            update_dir,
            f"{run_number}",
        )  # Add this to save file in specif directory
...
def get_env(
    env_name,
):  # , seed): # REMOVE seed from argument, only added from testing purposes
...
def get_td3_taylor_agent(
    *,
    ...
    return TD3_Taylor(
        ...
        action_cov=td3_action_cov,
        grad_action=grad_action,
        grad_state=grad_state,
        update_order=td3_update_order,
        state_cov=td3_state_cov,
        expl_noise=td3_expl_noise,
        norm_grad_terms=norm_grad_terms,
    )
...
def get_imagination(
    ...
    *,
    ...
    return SingleStepImagination(
        model,
        initial_states,
        n_actors=policy_actors,
        model_sampling_type=model_sampling_type,
        grad_state=grad_state,
...
@ex.capture
def get_buffer(
    d_state, d_action, n_total_steps, normalize_data, device, data_buffer_size
):
...
# This class is not used, just kept in case need to sample full buffer transitions
class BufferTransitionsProvider:
    ...
    def get_training_transitions(self, agent):
        ...
        states, actions, next_states = [
            x[idx].to(self.device) for x in [states, actions, next_states]
        ]
...
class ImaginationTransitionsProvider:
    ...
    def get_training_transitions(self, agent):
        states, actions, next_states = self.imagination.many_steps(
            agent
        )  # This returns a batch of initial states and the corresponding "imagined" next states with the actions
        dones = self.is_done(next_states)
        return states, actions, next_states, dones
...
def get_training_data_provider(model, buffer, is_done):
    (
        ...
        _,
        _,
    ) = buffer.view()  # This returns all inital states so far in the buffer
    ...
    *,
    ...
    for img_step_i in range(1, policy_training_n_iters + 1):
        ...
        for img_update_i in range(1, policy_training_n_updates_per_iter + 1):
            raw_action, q_loss, q_grad_loss, _, pi_loss = agent.update(
                states, actions, rewards, next_states, masks=~dones
            )  # Key method call to the critic and actor update
    ...
    logger.debug(
        f"{ex.step_i:6d} | {mode_task} | pi_loss: {pi_loss:6.3f}; q_loss: {q_loss:6.3f}"
    )
...
def model_train_epoch(
    model, buffer, optimizer, model_batch_size, model_training_grad_clip
):
    losses = []  # stores loss after each minibatch gradient update
    for states, actions, state_deltas in buffer.train_batches(
        ensemble_size=model.ensemble_size, batch_size=model_batch_size
    ):
        optimizer.zero_grad()
        ...
        torch.nn.utils.clip_grad_value_(
            model.parameters(), clip_value=model_training_grad_clip
        )
        optimizer.step()
...
def train_model(
    ...
    while batch_i < n_target_batches:
        ...
        logger.log(
            5,
            f"{ex.step_i:6d} | {mode} | batch {batch_i:3d} | model training loss: {loss:.2f}",
        )
...
@ex.capture
def reward_model_train_epoch(
    reward_model, buffer, optimizer, model_batch_size, model_training_grad_clip
):
    losses = []  # stores loss after each minibatch gradient update
    for states, actions, rewards, state_deltas in buffer.train_batches_rwd(
        ensemble_size=1, batch_size=model_batch_size
    ):
        ...
        loss.backward()
        torch.nn.utils.clip_grad_value_(
            reward_model.parameters(), clip_value=model_training_grad_clip
        )
        optimizer.step()
...
def train_reward_model(
    reward_model, optimizer, buffer, mode, model_training_n_batches, *, _run
):
    ...
    batch_i = 0
    while batch_i < n_target_batches:
        losses = reward_model_train_epoch(
            reward_model=reward_model, buffer=buffer, optimizer=optimizer
        )
        batch_i += len(losses)
        loss = np.mean(losses)
        logger.log(
...
def evaluate_on_task(
    ...
    *,
    ...
    agent = to_deterministic_agent(
        agent
    )  # This ensures evalutation performance are based on the deterministic (optimal) action
    ...
    for ep_i in range(1, n_eval_episodes_per_policy + 1):
        ...
        logger.log(
            15,
            f"{ex.step_i:6d} | {context} | t:{task_name} | episode {ep_i} | score: {ep_return:5.2f}",
        )
        ex.mlog.add_scalars(
            f"{context}/{task_name}/episode/",
            {"return": ep_return, "length": ep_len},
            ep_i=ep_i,
        )
...
def evaluate_on_tasks(agent, model, buffer, task_name, context):
    ...
    logger.info(
        f"{ex.step_i:6d} | {context} | t:{task_name} | avg. return: {avg_ep_return:5.2f}+-{std_ep_return:5.2f}"
    )
    ex.mlog.add_scalars(
        f"{context}/{task_name}/avg_episode/",
        {"return": avg_ep_return, "length": avg_ep_length},
    )
...
class MainTrainingLoop:
    ...
    @ex.capture
    def _common_setup(
        self, *, dump_dir, checkpoint, restart_checkpoint, checkpoint_freq, _run
    ):
    ...
    def train(
        ...
        *,
        ...
        behavioral_agent = (
            self.random_agent if ex.step_i <= n_warm_up_steps else self.agent
        )
        with torch.no_grad():
            action = (
        ...
        if (
            model_training_freq is not None
            and model_training_n_batches > 0
            and ex.step_i % model_training_freq == 0
        ):
        ...
        if eval_freq is not None and ex.step_i % eval_freq == 0:
            self.last_avg_eval_score = evaluate_on_tasks(
                agent=self.agent,
                model=self.model,
                buffer=self.buffer,
                task_name=self.task_name,
        ...
        if (
            ex.step_i % self.checkpoint_freq == 0
            and self.checkpoint
        ...
        return DotMap(
            ...
            action_abs_mean=action.abs()
            .mean()
            .item(),  # This is just for regression tests
            step_i=ex.step_i,
        )
