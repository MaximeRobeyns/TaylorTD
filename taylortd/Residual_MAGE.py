...
class Residual_MAGE(nn.Module):
    ...
    def get_action_with_logp(self, states):
        ...
        return (
            a,
            torch.ones(a.shape[0], device=a.device) * np.inf,
        )  # inf: should not be used
    ...
    def update(self, states, actions, logps, rewards, next_states, masks):
        ...
        critic_loss, standard_loss, gradient_loss = (
        ...
        if self.tdg_error_weight != 0:
            ...
            gradient_loss = torch.mean(
                gradients_error_norms
            )  # 0.5 * loss_fn(gradients_error_norms, zero_targets)
        ...
        ):  #% self.policy_delay == 0: # For residual try to update policy every iteration
            # Compute actor loss
            q = self.critic(
        ...
        return (
            ...
            standard_loss.item(),
            self.tdg_error_weight * gradient_loss.item(),
