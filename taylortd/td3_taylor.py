...
from taylortd.ddpg import Actor
from taylortd.radam import RAdam
from taylortd.models import ParallelLinear, get_activation
...
class ActionValueFunction(nn.Module):
    def __init__(self, d_state, d_action, n_layers, n_units, activation):
        ...
        if n_layers == 0:
            ...
            layers = [
                ParallelLinear(d_state + d_action, n_units, ensemble_size=2),
                get_activation(activation),
            ]
            for lyr_idx in range(1, n_layers):
                layers += [
                    ParallelLinear(n_units, n_units, ensemble_size=2),
                    get_activation(activation),
                ]
...
class TD3_Taylor(nn.Module):
    def __init__(
        ...
        self.critic = ActionValueFunction(
            d_state, d_action, value_n_layers, value_n_units, value_activation
        ).to(device)
    ...
    def update(self, states, actions, rewards, next_states, masks):
        ...
        )  # both next_s and next_a carry a gradient, which should relate them to action
        ...
        term_1, action_term1, state_term1 = (
        ...
        term_1 = 0.5 * (
            loss_fn(q1_td_error, zero_targets)
            + loss_fn(q2_td_error, zero_targets)
        )

        if self.grad_action:

            # Compute the gradient of the TD-error with respect to the action
            dac1 = torch.autograd.grad(
                outputs=q1_td_error,
                inputs=actions,
            ...
            if not self.norm_grad_terms:
                action_term1 = -1 * (
                    torch.mean(inner_product_last_dim(dac1.detach(), dQa1))
                    + torch.mean(inner_product_last_dim(dac2.detach(), dQa2))
                )
            else:
                action_term1 = -1 * (
                    torch.mean(
                        inner_product_last_dim(dac1.detach(), dQa1)
                        / (
                            dac1.detach().norm(dim=1, keepdim=True)
                            * dQa1.detach().norm(dim=1, keepdim=True)
                        )
                    )
                    + torch.mean(
                        inner_product_last_dim(dac2.detach(), dQa2)
                        / (
                            dac2.detach().norm(dim=1, keepdim=True)
                            * dQa2.detach().norm(dim=1, keepdim=True)
                        )
                    )
                )
        ...
        if self.grad_state:
            ...
            if not self.norm_grad_terms:
                state_term1 = -1 * (
                    torch.mean(inner_product_last_dim(dsc1.detach(), dQs1))
                    + torch.mean(inner_product_last_dim(dsc2.detach(), dQs2))
                )

            else:
                state_term1 = -1 * (
                    torch.mean(
                        inner_product_last_dim(dsc1.detach(), dQs1)
                        / (
                            dsc1.detach().norm(dim=1, keepdim=True)
                            * dQs1.detach().norm(dim=1, keepdim=True)
                        )
                    )
                    + torch.mean(
                        inner_product_last_dim(dsc2.detach(), dQs2)
                        / (
                            dsc2.detach().norm(dim=1, keepdim=True)
                            * dQs2.detach().norm(dim=1, keepdim=True)
                        )
                    )
                )
        ...
        critic_loss = (
            term_1
            + self.action_cov * action_term1
            + self.state_cov * state_term1
        )
        ...
        if self.step_counter % self.policy_delay == 0:
            ...
            )  # originally in TD3 we had here q1 only
        ...
        return (
            raw_next_actions[0, 0].item(),
            term_1.item(),
            self.action_cov * action_term1.item(),
            self.state_cov * state_term1,
