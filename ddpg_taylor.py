...
class DDPG_Taylor(nn.Module):
    ...
    def update(self, states, actions, logps, rewards, next_states, masks):
        ...
        next_Q = self.critic_target(next_states, next_actions) # both next_s and next_a carry a gradient, which should relate them to action
        ...
        q_td_err_2 = q_td_error**2

        # Compute gradient of TD-error relative to actions
        # Shape: [batch, actions]
        dac = torch.autograd.grad(outputs=q_td_error, inputs=actions,
        ...
        dac_2 = inner_product_last_dim(dac, dac)
        assert dac_2.size(-1) == 1 
        
        td_loss = 0.5 * (loss_fn(q_td_err_2, zero_targets))
        ag_loss = 0.5 * (loss_fn(dac_2, zero_targets))    
        ...
        if self.grad_state:
                ...
                dsc = torch.autograd.grad(outputs=q_td_error, inputs=states,
                                           grad_outputs=torch.ones(q_td_error.size(), device=self.device),
                                           retain_graph=True, create_graph=True,
                                           only_inputs=True)[0].flatten(start_dim=1)#.norm(dim=1, keepdim=True)
                ...
                # Compute magnitude of gradient of TD relative to actions
                dsc_2 = inner_product_last_dim(dsc, dsc)
                assert dsc_2.size(-1) == 1 
                
                sg_loss = 0.5 * (loss_fn(dsc_2, zero_targets))
        ...
        if self.update_order == 2:
                ...
                batch_s = dac.size()[0]
                action_s = dac.size()[1]
                Hess_a = torch.zeros(batch_s,action_s,action_s, device=self.device) # The gradient dac has shape depending on action_s, which we further differentiate relative to action_s

                for i in range(action_s):

                        Hess_a[:,i,:] = torch.autograd.grad(dac[:,i],actions,grad_outputs=torch.ones(batch_s, device=self.device),
                                                             retain_graph=True, create_graph=True,
                                                             only_inputs=True)[0]

                # Compute the square of the Hessian        
                Hess_a2 = Hess_a @ Hess_a
               
                # Compute the trace
                trace_H = torch.sum(torch.diagonal(Hess_a2, dim1=1,dim2=2),dim=1, keepdim=True)

                aag_loss = 0.5 * (loss_fn(trace_H, zero_targets))
        else:
